...
impl Board {
    ...
    pub fn push_str(&mut self, move_str: &str) -> Result<(), &'static str> {
        ...
        let moved_pc = self
            .piece_at(from_sq)
            .ok_or("No piece at the origin square")?;

        if moved_pc.color_of() != self.ctm {
            return Err("Moved piece of the wrong color.");
        }

        let promo_pt = move_str
            .chars()
            .nth(4)
            .map(|pt_char| Piece::try_from(pt_char))
            .transpose()?
            .map(|pc| pc.type_of());

        let flag = if self.piece_at(to_sq).is_some() {
            match promo_pt {
                Some(PieceType::Queen) => MoveFlags::PcQueen,
                Some(PieceType::Knight) => MoveFlags::PcKnight,
                Some(PieceType::Bishop) => MoveFlags::PcBishop,
                Some(PieceType::Rook) => MoveFlags::PcRook,
                None => MoveFlags::Capture,
                _ => {
                    unreachable!()
                }
            }
            ...
            match promo_pt {
                Some(PieceType::Queen) => MoveFlags::PrQueen,
                Some(PieceType::Knight) => MoveFlags::PrKnight,
