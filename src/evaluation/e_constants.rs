...
use crate::types::square::{N_SQUARES, SQ, Direction};
use crate::types::piece::{N_PIECES, Piece};
...
pub fn piece_sq_value(pc: Piece, sq: SQ) -> Score {
    unsafe {
        PIECE_TABLES[pc.index()][sq.index()]
    }
}

fn init_pawn_shields(pawn_shields: &mut [[BitBoard; N_SQUARES]; N_COLORS]) {
    for sq in SQ::A1..=SQ::H8 {
        pawn_shields[Color::White.index()][sq.index()] = sq.bb().shift(Direction::North, 1) | sq.bb().shift(Direction::NorthEast, 1) | sq.bb().shift(Direction::NorthWest, 1);
        pawn_shields[Color::Black.index()][sq.index()] = sq.bb().shift(Direction::South, 1) | sq.bb().shift(Direction::SouthEast, 1) | sq.bb().shift(Direction::SouthWest, 1);
    }
}

fn init_piece_values(piece_values: &mut [Score; N_PIECES], piece_tables: &mut [[Score; N_SQUARES]; N_PIECES]) {
    for pc in Piece::WhitePawn..=Piece::WhiteKing {
        ...
        for sq in SQ::A1..=SQ::H8 {
            ...
            piece_tables[pc.flip().index()][sq.index()] = -PIECE_TYPE_TABLES[pc.index()][sq.square_mirror().index()];
        }
    }
}
